...
void exchange_qp(unsigned int master_rank, unsigned int slave_rank, unsigned int local_rank, std::vector<ACCL::rank_t> &ranks, ACCL::CoyoteDevice* device)
{
...
void test_copy(ACCL::ACCL &accl, options_t &options){
	std::cout << "Start copy test..." << std::endl<<std::flush;
	unsigned int count = options.count;
	auto op_buf = accl.create_coyotebuffer<float>(count, ACCL::dataType::float32);
	auto res_buf = accl.create_coyotebuffer<float>(count, ACCL::dataType::float32);
	int errors = 0;
	if (options.count*sizeof(ACCL::dataType::float32) > options.rxbuf_size){
...
void test_sendrcv(ACCL::ACCL &accl, options_t &options) {
	...
	if (options.count*sizeof(ACCL::dataType::int32) > options.rxbuf_size){
		std::cout<<"experiment size larger than buffer size, exiting..."<<std::endl;
		return;
	}

	auto op_buf = accl.create_coyotebuffer<int>(bufsize, ACCL::dataType::int32);
	...
	for (int n = 0; n < options.nruns; n++)
	{
		...
		for (int i = 0; i < bufsize; i++) op_buf.get()->buffer()[i] = (mpi_rank == 0) ? (i + n) : -1;
		op_buf.get()->buffer()[0] = (int) (5 + n);
		...
		if (mpi_rank == 0) {
			// send
			req = accl.send(*op_buf, bufsize, 1, ACCL::TAG_ANY, ACCL::GLOBAL_COMM, true, ACCL::dataType::none, true); // most default send from 0 to 1
			accl.wait(req, 1000ms);

		} else if (mpi_rank == 1) {
			// receive
			req = accl.recv(*op_buf, bufsize, 0, ACCL::TAG_ANY, ACCL::GLOBAL_COMM, true, ACCL::dataType::none, true); // most default recv to 1 from 0
			accl.wait(req, 1000ms);
		}
		
		auto end = std::chrono::high_resolution_clock::now();
		durationUs = (std::chrono::duration_cast<std::chrono::nanoseconds>(end-start).count() / 1000.0);
		tput = (options.count*sizeof(ACCL::dataType::int32)*8.0)/(durationUs*1000.0);
		...
		if (mpi_rank == 0 || mpi_rank == 1){
			...
			tput = (options.count*sizeof(ACCL::dataType::int32)*8.0)/(durationUs*1000.0);
			if(durationUs > 1.0){
				ACCL::accl_log(mpi_rank, format_log("sendrecv", options, durationUs, tput));
			}
		}
		...
		ACCL::debug(accl.dump_eager_rx_buffers(false));
	}
...
void test_bcast(ACCL::ACCL &accl, options_t &options, int root) {
	...
	auto op_buf = accl.create_coyotebuffer<int>(count, ACCL::dataType::int32);
	...
	for (int n = 0; n < options.nruns; n++)
	{
		...
		auto start = std::chrono::high_resolution_clock::now();
		ACCL::ACCLRequest* req = accl.bcast(*op_buf, count, root, ACCL::GLOBAL_COMM, true, true, ACCL::dataType::none, true);
		accl.wait(req, 1000ms);
		auto end = std::chrono::high_resolution_clock::now();
		durationUs = (std::chrono::duration_cast<std::chrono::nanoseconds>(end-start).count() / 1000.0);
		std::cout<<"host measured durationUs:"<<durationUs<<std::endl;
		...
		durationUs = (double)accl.get_duration(req)/1000.0;
		if(durationUs > 1.0){
			ACCL::accl_log(mpi_rank, format_log("bcast", options, durationUs, 0));
		}
...
void test_scatter(ACCL::ACCL &accl, options_t &options, int root) {
	...
	if (options.count*mpi_size*sizeof(ACCL::dataType::int32) > options.rxbuf_size){
		std::cout<<"experiment size larger than buffer size, exiting..."<<std::endl;
		return;
	}

	auto op_buf = accl.create_coyotebuffer<int>(count * mpi_size, ACCL::dataType::int32);
	auto res_buf = accl.create_coyotebuffer<int>(count, ACCL::dataType::int32);
	...
	for (int n = 0; n < options.nruns; n++)
	{
		...
		std::cout<<"Pass accl barrier"<<std::endl;
		auto start = std::chrono::high_resolution_clock::now();
		ACCL::ACCLRequest* req = accl.scatter(*op_buf, *res_buf, count, root, ACCL::GLOBAL_COMM, true, true, ACCL::dataType::none, true);
		accl.wait(req, 1000ms);
		auto end = std::chrono::high_resolution_clock::now();
		durationUs = (std::chrono::duration_cast<std::chrono::nanoseconds>(end-start).count() / 1000.0);
		std::cout<<"host measured durationUs:"<<durationUs<<std::endl;
		...
		durationUs = (double)accl.get_duration(req)/1000.0;
		if(durationUs > 1.0){
			ACCL::accl_log(mpi_rank, format_log("scatter", options, durationUs, 0));
		}
...
void test_gather(ACCL::ACCL &accl, options_t &options, int root) {
	...
	if (options.count*mpi_size*sizeof(ACCL::dataType::float32) > options.rxbuf_size){
		std::cout<<"experiment size larger than buffer size, exiting..."<<std::endl;
		return;
	}
	...
	if (mpi_rank == root) {
		res_buf = accl.create_coyotebuffer<float>(count * mpi_size, ACCL::dataType::float32);
	} else {
	...
	for (int n = 0; n < options.nruns; n++)
	{
		...
		std::cout<<"Pass accl barrier"<<std::endl;
		auto start = std::chrono::high_resolution_clock::now();
		ACCL::ACCLRequest* req = accl.gather(*op_buf, *res_buf, count, root, ACCL::GLOBAL_COMM, true, true, ACCL::dataType::none, true);
		accl.wait(req, 1000ms);
		...
		durationUs = (double)accl.get_duration(req)/1000.0;
		if(durationUs > 1.0){
			ACCL::accl_log(mpi_rank, format_log("gather", options, durationUs, 0));
		}
...
void test_allgather(ACCL::ACCL &accl, options_t &options) {
	...
	auto op_buf = accl.create_coyotebuffer<float>(count, ACCL::dataType::float32);
	auto res_buf = accl.create_coyotebuffer<float>(count * mpi_size, ACCL::dataType::float32);
	...
	for (int n = 0; n < options.nruns; n++)
	{
		...
		std::cout<<"Pass accl barrier"<<std::endl;
		auto start = std::chrono::high_resolution_clock::now();
		ACCL::ACCLRequest* req = accl.allgather(*op_buf, *res_buf, count, ACCL::GLOBAL_COMM, true, true, ACCL::dataType::none, true);
		accl.wait(req, 1000ms);
		auto end = std::chrono::high_resolution_clock::now();
		durationUs = (std::chrono::duration_cast<std::chrono::nanoseconds>(end-start).count() / 1000.0);
		std::cout<<"host measured durationUs:"<<durationUs<<std::endl;
		...
		durationUs = (double)accl.get_duration(req)/1000.0;
		if(durationUs > 1.0){
			ACCL::accl_log(mpi_rank, format_log("allgather", options, durationUs, 0));
		}
...
void test_reduce(ACCL::ACCL &accl, options_t &options, int root,
	...
	for (int n = 0; n < options.nruns; n++)
	{
		...
		std::cout<<"Pass accl barrier"<<std::endl;
		auto start = std::chrono::high_resolution_clock::now();
		ACCL::ACCLRequest* req = accl.reduce(*op_buf, *res_buf, count, root, function, ACCL::GLOBAL_COMM, true, true, ACCL::dataType::none, true);
		accl.wait(req, 1000ms);
		auto end = std::chrono::high_resolution_clock::now();
		durationUs = (std::chrono::duration_cast<std::chrono::nanoseconds>(end-start).count() / 1000.0);
		std::cout<<"host measured durationUs:"<<durationUs<<std::endl;
		...
		durationUs = (double)accl.get_duration(req)/1000.0;
		if(durationUs > 1.0){
			ACCL::accl_log(mpi_rank, format_log("reduce", options, durationUs, 0));
		}
...
void test_allreduce(ACCL::ACCL &accl, options_t &options,
	...
	for (int n = 0; n < options.nruns; n++)
	{
		...
		std::cout<<"Pass accl barrier"<<std::endl;
		auto start = std::chrono::high_resolution_clock::now();
		ACCL::ACCLRequest* req = accl.allreduce(*op_buf, *res_buf, count, function, ACCL::GLOBAL_COMM, true, true, ACCL::dataType::none, true);
		accl.wait(req, 1000ms);
		auto end = std::chrono::high_resolution_clock::now();
		durationUs = (std::chrono::duration_cast<std::chrono::nanoseconds>(end-start).count() / 1000.0);
		std::cout<<"host measured durationUs:"<<durationUs<<std::endl;
		...
		durationUs = (double)accl.get_duration(req)/1000.0;
		if(durationUs > 1.0){
			ACCL::accl_log(mpi_rank, format_log("allreduce", options, durationUs, 0));
		}
...
void test_accl_base(options_t options)
{
	...
	if (options.hardware)
	{
		if (options.udp)
		{
			ACCL::debug("ERROR: we don't support UDP for now!!!");
			exit(1);
		}
		...
		if (options.protoc == 0){
			...
			accl.get()->initialize(ranks, mpi_rank,
				mpi_size+3, options.rxbuf_size, 4096*1024, 4096*1024*2, eagerBufs);
		} else if (options.protoc == 1){
		...
		ACCL::debug("unsupported situation!!!");
		exit(1);
	}
	...
	/*if(options.test_mode == ACCL_REDUCE || options.test_mode == 0){
		...
		MPI_Barrier(MPI_COMM_WORLD);
		int root = 0;
		test_reduce(*accl, options, root, ACCL::reduceFunction::SUM);
		ACCL::debug(accl->dump_communicator());
		ACCL::debug(accl->dump_eager_rx_buffers(false));
	}
	if(options.test_mode == ACCL_ALLREDUCE || options.test_mode == 0){
		ACCL::debug(accl->dump_eager_rx_buffers(false));
		MPI_Barrier(MPI_COMM_WORLD);
		test_allreduce(*accl, options, ACCL::reduceFunction::SUM);
		ACCL::debug(accl->dump_communicator());
		ACCL::debug(accl->dump_eager_rx_buffers(false));
	}*/
