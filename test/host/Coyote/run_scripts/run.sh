		...
		for N_ELE in ${N_ELEMENTS[@]}; do
			for H in ${HOST[@]}; do
				for P in ${PROTOC[@]}; do
					...
					/mnt/scratch/zhe/mpich/install/bin/mpirun -n $NP -f $HOST_FILE --iface ens4f0 -outfile-pattern "./accl_log/rank_%r_M_${MODE}_N_${N}_H_${H}_P_${P}_stdout" -errfile-pattern "./accl_log/rank_%r_M_${MODE}_N_${N}_H_${H}_P_${P}_stdout" $EXEC $ARG -z $H -y $MODE -c $N -l $FPGA_FILE -p $P -n $NRUN &
					SLEEPTIME=2
					sleep $SLEEPTIME
